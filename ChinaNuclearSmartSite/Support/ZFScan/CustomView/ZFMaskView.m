...
//    self.hintLabel.backgroundColor = [UIColor redColor];
    self.hintLabel.text = @"请对准二维码或条形码";
    ...
    //竖屏
        ...
        //提示框
        self.hintLabel.frame = CGRectMake(0, 0, self.frame.size.width * ZFScanRatio, 30);
        if (SCREEN_WIDTH == 414) {
            self.hintLabel.center = CGPointMake(self.maskView.center.x, self.maskView.frame.size.height / 2 + 162);
        }else if (SCREEN_WIDTH == 375){
            self.hintLabel.center = CGPointMake(self.maskView.center.x, self.maskView.frame.size.height / 2 + 142);
        }else{
            self.hintLabel.center = CGPointMake(self.maskView.center.x, self.maskView.frame.size.height / 2 + 123);
        }
